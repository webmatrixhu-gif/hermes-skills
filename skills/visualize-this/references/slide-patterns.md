...
**Step 4 — Plan images only when they serve the request.** If the user asked for illustration-heavy output, or imagery is integral to an explicitly requested deck, use Hermes `image_generate` for a small number of deliberate images. Good targets are a 16:9 title background, one full-bleed visual anchor, or a 1:1 conceptual illustration. Generate them before writing HTML, then embed them as data URIs or deliver them alongside the page. Structural and data-heavy decks should normally use CSS, SVG, Mermaid, and charts instead. If image generation is unavailable or unnecessary, use CSS and SVG without treating that as a failure.
...
Full-viewport hero. Background treatment via gradient, texture, or an intentionally generated image. 80–120px display type.
...
Background image (generated or supplied) or a CSS treatment dominates the viewport. Text overlay with gradient scrim ensuring contrast. Zero slide padding.
...
**Hermes image generation:** Use `image_generate` only when imagery is part of the user's requested tone or materially improves the deck. Do not generate images merely because the tool is available. Good targets, in priority order:

1. **Title slide:** a 16:9 background that establishes the deck's visual tone.
2. **Full-bleed slide:** an immersive visual anchor whose style matches the chosen preset.
3. **Conceptual content slide:** a 1:1 illustration in the `.slide__aside` area when an abstract idea benefits from one.

Generate images before writing the final HTML. Use `image_generate` with the page palette, mood, composition, and aspect ratio in the prompt. Embed the returned file as a data URI only when a truly standalone file is required; otherwise keep it as an adjacent asset and deliver both files.
...
**When image generation is unavailable or unnecessary:** Use CSS gradients, SVG decoration, charts, or diagrams. The deck must stand on its own without generated imagery; images enhance the composition but do not carry the explanation.
...
**When to skip images:** Pure structural or data-heavy decks such as code reviews and table comparisons usually do not need generated images.
